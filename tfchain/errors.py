...
class CurrencyNegativeValue(Exception):
    ...
    def __init__(self, value):
        super().__init__(
            "currency has to be at least 0, while value {} is negative".format(str(value)))
        self._value = value
...
class ExplorerError(Exception):
    ...
    def __init__(self, message, endpoint):
        ...
        if not isinstance(endpoint, str):
            raise TypeError(
                "invalid endpoint, expected it to be of type str not {}".format(type(endpoint)))
...
class ExplorerNotAvailable(ExplorerError):
    ...
    def __init__(self, message, endpoint, addresses):
        ...
        if not isinstance(addresses, list):
            raise TypeError(
                "invalid addresses, expected it to be of type list not {}".format(type(addresses)))
...
class ExplorerInvalidResponse(ExplorerError):
    ...
    def __init__(self, message, endpoint, response):
        ...
        if not isinstance(response, dict):
            raise TypeError(
                "invalid response, expected it to be of type dict not {}".format(type(response)))
...
class AtomicSwapContractSpent(AtomicSwapContractError):
    ...
    def __init__(self, contract, transaction):
        ...
        super().__init__(message="atomic swap contract has already been spent in transaction {}".format(
            str(txid)), contract=contract)
...
class ThreeBotInactive(Exception):
    ...
    def __init__(self, identifier, expiration):
        # FIXME: HRDatetime port.
        # jstime.epoch2HRDateTime(expiration)))
        super().__init__("3Bot {} is inactive since {}".format(str(identifier), expiration))
        self._identifier = identifier
